// import makeElement from './appMakeElement.js'
...
/*   const header = makeElement('header', 'header')
  const headerContainer = makeElement('div', ('container','header__container'))
  const headerCity = makeElement('div', 'header__city')
  const headerUnits = makeElement('div', 'header__units')
  const cityChange = makeElement('button', 'city__changebtn-reset', 'Сменить город')
  const cityLocation = makeElement('button', ('city__location','btn-reset'), 'Моё местоположение')
  const cityName = makeElement('h1', 'city__name', city)
  const unitsC = makeElement('button', 'units__c' ('btn-reset', 'unit-current'), 'C')
  const unitsF = makeElement('button', ('units__f', 'btn-reset'), 'F')
  const cityInner = makeElement('div', 'city__inner')
  const searchBlock = makeElement('div', 'search')
  const searchInput = makeElement('input', 'search_input')
  const searchBtn = makeElement('button', 'search_btn', 'ok')
  const errorBlock = makeElement('p', 'search__error')
